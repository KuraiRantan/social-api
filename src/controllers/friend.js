...
const sendFriend = async (req, res) => {
    ...
    try {
        let [request, created] = await db.Friend.findOrCreate({
            where: {
                [Op.or]: [{ user_id: user.id, friend_id }, { user_id: friend_id, friend_id: user.id }]
            },
            include: [
                {
                    model: db.User,
        ...
        if (!created && !operation) return res.status(400).json({ message: 'Solicitud enviada', error: 'Ya se ha enviado la solicitud' })
        if (!created) {
            const requestStatus = request.toJSON().status;

            if (
                (requestStatus === 'pending' && operation === 'acepted') ||
                (requestStatus === 'acepted' || requestStatus === 'pending' && operation === 'refused')
            ) {
        ...
        const messageNotification = `${user.firstName} ${user.lastName} ha ${operation === 'pending' ? 'send' : operation} solicitud de amistad`;
        ...
        if (socket) {
            socket.emit('notificationRequestFriend', { notification: notificationFriend, friend: { ...request } });
        }
